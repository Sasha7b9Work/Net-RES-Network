...
HAL_StatusTypeDef MAX44009_WriteReg(uint8_t addr, uint8_t value)
{
	aTxBuffer[0] = addr;
	aTxBuffer[1] = value;
	HAL_StatusTypeDef HAL_result;
	HAL_result = I2C_WriteBuffer(hi2c1, MAX44009_ADDR, 2);	//1 Byte
...
// return:
//   register value
uint8_t MAX44009_ReadReg(uint8_t addr) {

	aTxBuffer[0] = addr;
	HAL_StatusTypeDef HAL_result;
	HAL_result = I2C_WriteBuffer(hi2c1, MAX44009_ADDR, 1);	//1 Byte
	while (HAL_I2C_GetState(&hi2c1) != HAL_I2C_STATE_READY);
	I2C_ReadBuffer(hi2c1, MAX44009_ADDR, 1);	//1 Byte
	while (HAL_I2C_GetState(&hi2c1) != HAL_I2C_STATE_READY);

	uint8_t value = aTxBuffer[0];
	return value;
}
