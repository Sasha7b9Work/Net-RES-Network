...
uint16_t tmpVal;
void pack2BytesInt(uint8_t index, double bytes)
{
	tmpVal = bytes;
	memcpy(&radio_buf[index], &tmpVal, 2);
}
...
float ADC_Convert(uint16_t value)
{
	...
	Vadc = value*3.3/4095.0;
	Rs = (5 - Vadc) / ( 10 * Vadc);
	var = (116 * powf(Rs/12.0, -1.2) / 1000.0);
...
int main(void)
{
  ...
  while (1)
  {
		...
		co2 = ADC_Convert( HAL_ADC_GetValue(&hadc1));
		HAL_ADC_Stop(&hadc1);
